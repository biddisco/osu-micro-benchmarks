...
void print_help_message (int rank)
{
...
#if MPI_VERSION >= 3
    printf("Usage: ./osu_get_bw -w <win_option>  -s < sync_option> \n");
    printf("win_option:\n");
    ...
    printf("\n");
#else 
    printf("Usage: ./osu_get_bw -s < sync_option> \n");
#endif
...
void allocate_memory(int rank, char *sbuf, int size, WINDOW type, MPI_Win *win)
{
#if MPI_VERSION >= 3
    MPI_Status  reqstat;
    ...
    switch (type){
        case WIN_CREATE:
            MPI_CHECK(MPI_Win_create(sbuf, WINDOW_SIZE_LARGE*size, 1, MPI_INFO_NULL, MPI_COMM_WORLD, win));
            break;
        case WIN_DYNAMIC:
            MPI_CHECK(MPI_Win_create_dynamic(MPI_INFO_NULL, MPI_COMM_WORLD, win));
            MPI_CHECK(MPI_Win_attach(*win, (void *)sbuf, WINDOW_SIZE_LARGE*size));
            MPI_CHECK(MPI_Get_address(sbuf, &sdisp_local));
            if(rank == 0){
            ...
            break;
        default:
            MPI_CHECK(MPI_Win_allocate(WINDOW_SIZE_LARGE*size, 1, MPI_INFO_NULL, MPI_COMM_WORLD, sbuf, win));
            break;
    }
#else
    MPI_CHECK(MPI_Win_create(sbuf, WINDOW_SIZE_LARGE*size, 1, MPI_INFO_NULL, MPI_COMM_WORLD, win));
...
void run_get_with_flush_local (int rank, WINDOW type)
{
    ...
    for (size = 1; size <= MAX_SIZE; size = size * 2) {
        allocate_memory(rank, sbuf, size, type, &win);
