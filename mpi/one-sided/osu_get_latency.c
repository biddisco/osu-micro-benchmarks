#define BENCHMARK "OSU MPI_Get latency Test"
/*
...
void print_help_message (int rank)
{
...
#if MPI_VERSION >= 3
    printf("Usage: ./osu_get_latency -w <win_option>  -s < sync_option> \n");
    printf("win_option:\n");
    ...
    printf("\n");
#else
    printf("Usage: ./osu_get_latency -s < sync_option> \n");
#endif
...
void allocate_memory(int rank, char *sbuf, int size, WINDOW type, MPI_Win *win)
{
    ...
    switch (type){
        case WIN_CREATE:
            MPI_CHECK(MPI_Win_create(sbuf, size, 1, MPI_INFO_NULL, MPI_COMM_WORLD, win));
            break;
        case WIN_DYNAMIC:
            MPI_CHECK(MPI_Win_create_dynamic(MPI_INFO_NULL, MPI_COMM_WORLD, win));
            MPI_CHECK(MPI_Win_attach(*win, (void *)sbuf, size));
            ...
            break;
        default:
            MPI_CHECK(MPI_Win_allocate(size, 1, MPI_INFO_NULL, MPI_COMM_WORLD, sbuf, win));
            break;
    }
#else
    MPI_CHECK(MPI_Win_create(sbuf, size, 1, MPI_INFO_NULL, MPI_COMM_WORLD, win));
